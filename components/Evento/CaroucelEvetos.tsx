...
export default function CaroucelEvetos() {
  ...
  useEffect(() => {
    const loadData = async () => {
      const response = await fetch(`${API_URL}/eventosjoinuss/list/externos`, {
        method: "GET",
      });
  ...
  return (
    <div className="w-full flex flex-col gap-5">
      <h1 className="text-5xl md:text-6xl 2xl:text-8xl text-white text-center bebas-neue">
        {translations[language].footer_eventos_externos}
      </h1>
              ...
              <>
                {data.map((evento: Eventos, index: number) => (
                  <SwiperSlide key={index}>
                    <div className="w-full flex flex-col gap-3">
                      <Link href={evento.descripcion} target="_blank" className="w-full flex justify-end">
                        <div className="w-full h-[250px] 2xl:h-[350px] bg-slate-400 !relative rounded-lg overflow-hidden">
                          {evento.image && (
                            <Image
                              unoptimized
                              src={evento.image}
                          ...
                          <div className="absolute bottom-1 right-1 flex flex-row gap-1 z-20">
                            ...
                            <span className="px-3 py-1 text-xs 2xl:text-sm text-white bg-black/70 rounded-lg">
                              {formateDate(evento.fecha_creacion)}
                            </span>
                          </div>
                        </div>
                      </Link>
                      <div className="w-full relative text-white -mt-10 z-10 flex flex-col gap-3 md:gap-5">
                        ...
                        <h1 className="font-semibold text-2xl 2xl:text-4xl">{evento.nombre}</h1>
                        {/* <div className="w-full flex justify-end">
