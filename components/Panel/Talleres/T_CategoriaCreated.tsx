...
export default function T_CategoriaCreated({
  onClose,
  edit = false,
  nombre_p = "",
  id = 0,
  ...
  const onSubmit = async () => {
    const dataJson: T_Categoria_I = { categoria: nombre };
    ...
    if (edit == false) {
      response = await fetch(`${API_URL}/talleres/create/categoria`, {
        method: "POST",
        headers: {
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={(open) => closeDialog(open)}>
        <DialogTrigger
          className={`flex flex-row gap-1 items-center text-sm ${edit == false ? "lg:text-xl" : ""
            }`}
        >
          {edit == false ? (
            <>
